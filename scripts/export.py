...
def main(argv):
    ...
    # # # --- HAPTIC EXPORT FIX: Re-enable PQMF for the encoder input ---
    # # # The encoder (EncoderV2) is built to expect multi-band input (e.g., 16 channels),
    # # # so we must re-initialize and enable the PQMF module for encoding only.
    # # if pretrained.input_mode == "pqmf":
    # #     # Need to import PQMF from rave.pqmf if not already done at the top
    # #     from rave.pqmf import PQMF

    # #     # --- CRITICAL: Use the required arguments for PQMF initialization ---
    # #     # Assuming N_BAND=16 (default) and ATTENUATION=100 (default) based on typical RAVE configs.
    # #     N_BAND = 16
    # #     ATTENUATION = 100

    # #     pretrained.pqmf = PQMF(n_band=N_BAND, attenuation=ATTENUATION, n_channels=pretrained.n_channels)
