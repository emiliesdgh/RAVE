...
class UpsampleLayer(nn.Module):
    ...
    def __init__(
        ...
        if ratio > 1:
            net.append(normalization(cc.ConvTranspose1d(in_dim, out_dim, 2 * ratio, stride=ratio, padding=ratio // 2)))
        else:
            net.append(normalization(cc.Conv1d(in_dim, out_dim, 3, padding=cc.get_padding(3))))
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        for i, r in enumerate(ratios):
            ...
            net.append(ResidualStack(out_dim, cumulative_delay=net[-1].cumulative_delay))

        self.net = cc.CachedSequential(*net)

        wave_gen = normalization(cc.Conv1d(out_dim, data_size * n_channels, 7, padding=cc.get_padding(7)))
...
class GeneratorV2(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:  # 04.11.2025
        ...
        y_high_rate = self.high_rate_waveform_module(x)

        # 2. Generate haptic waveform (haptic_pred)
        # Apply amplitude modulation to audio BEFORE downsampling if enabled
        if self.amplitude_modulation:
            y_audio, amplitude = y_high_rate.split(y_high_rate.shape[1] // 2, 1)
            y_audio = y_audio * torch.sigmoid(amplitude)
        else:
            y_audio = y_high_rate
...
class WasserteinEncoder(nn.Module):
    ...
    def reparametrize(self, z):
        ...
        if self.noise_augmentation:
            noise = torch.randn(z.shape[0], self.noise_augmentation, z.shape[-1]).type_as(z)
...
class DiscreteEncoder(nn.Module):
    ...
    def __init__(self, encoder_cls, vq_cls, num_quantizers, noise_augmentation: int = 0, n_channels: int = 1):
        super().__init__()
        self.encoder = encoder_cls(n_channels=n_channels)
        self.rvq = vq_cls()
...
class AdaptiveInstanceNormalization(nn.Module):
    ...
    def update(self, target: torch.Tensor, source: torch.Tensor, num_updates: torch.Tensor) -> None:
        bs = source.shape[0]
